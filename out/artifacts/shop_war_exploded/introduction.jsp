...
$(function(){
	...
	$("#gotoShoppingCart").click(function(){
		...
		// 1.得到数据(商品id,数量)
		var goodsInfoId = "${param.id}";
		var count = $("#value").val();
		
		console.info(goodsInfoId);
		console.info(count);
		
		var param = new Object(); // 先创建一个对象
		param.id = goodsInfoId; // 把属性封装到对象里面
		param.count = count; 
		$.get("ShoppingServlet?action=gotoShoopingCart",param); // 调用后台servlet添加到购物车中
	});
